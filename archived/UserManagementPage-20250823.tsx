...
import { ConfirmModal } from '@/components/common/ModernModalSystem';
import { SimpleToast as Toast } from '@/components/common/Toast';
...
export function UserManagementPage() {
  ...
  const columns = useMemo(() => [
    // 选择列
    ...(hasPermission(PERMISSIONS.USER_BATCH_OPERATION) ? [{
      id: 'select',
      header: ({ table }: any) => (
        <input
          type="checkbox"
  ...
  // Wrapper functions to handle different signatures  
  const handleCreateUserSubmit = useCallback(async (data: any) => {
    await createUser(data);
    // No return value needed for Promise<void>
  }, [createUser]);

  const handleUpdateUserSubmit = useCallback(async (data: any) => {
    if (editingUser && data.userId) {
  ...
  // 检查页面访问权限
  if (!hasAnyPermission([PERMISSIONS.USER_VIEW, PERMISSIONS.USER_CREATE, PERMISSIONS.USER_UPDATE])) {
    return (
        ...
        <div className="card-body toolbar-compact pb-2">
          <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {hasPermission(PERMISSIONS.USER_BATCH_OPERATION) && selectedUsers.length > 0 && (
          ...
          <div className="card-body pt-0">
            <DataTable
              ...
              onRowSelectionChange={(selection) => {
                // Convert RowSelectionState to string array of user IDs
                const selectedIds = Object.keys(selection)
                  .filter(key => selection[key])
                  .map(index => users[parseInt(index)]?.id)
                  .filter(Boolean);
                setSelectedUsers(selectedIds);
              }}
              enableRowSelection={hasPermission(PERMISSIONS.USER_BATCH_OPERATION)}
              loading={loading}

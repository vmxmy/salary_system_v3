...
const StatisticsPage: React.FC = () => {
  ...
  const renderModuleNavigation = () => (
    <div className="card bg-base-100 shadow-sm border border-base-200 p-4">
      ...
      {/* 使用DaisyUI 5标准tabs组件，带有适当的ARIA属性 */}
      <div role="tablist" className="tabs tabs-lift tabs-lg w-full" aria-label="统计模块导航">
        <button 
          role="tab" 
          id="tab-dashboard"
          aria-controls="tabpanel-dashboard"
          aria-selected={pageState.activeModule === 'dashboard'}
          className={`tab flex items-center gap-2 ${
            pageState.activeModule === 'dashboard' ? 'tab-active' : ''
          }`}
        ...
        <button 
          role="tab" 
          id="tab-hr-stats"
          aria-controls="tabpanel-hr-stats"
          aria-selected={pageState.activeModule === 'hr-stats'}
          className={`tab flex items-center gap-2 ${
            pageState.activeModule === 'hr-stats' ? 'tab-active' : ''
          } ${!hasStatisticsPermission('hr-stats') ? 'tab-disabled opacity-50' : ''}`}
          onClick={() => hasStatisticsPermission('hr-stats') && setActiveModule('hr-stats')}
          disabled={!hasStatisticsPermission('hr-stats')}
          aria-disabled={!hasStatisticsPermission('hr-stats')}
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
        ...
        <button 
          role="tab" 
          id="tab-payroll-stats"
          aria-controls="tabpanel-payroll-stats"
          aria-selected={pageState.activeModule === 'payroll-stats'}
          className={`tab flex items-center gap-2 ${
            pageState.activeModule === 'payroll-stats' ? 'tab-active' : ''
          } ${!hasStatisticsPermission('payroll-stats') ? 'tab-disabled opacity-50' : ''}`}
          ...
          aria-disabled={!hasStatisticsPermission('payroll-stats')}
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
        ...
        <button 
          ...
          className={`tab flex items-center gap-2 ${
            pageState.activeModule === 'trends' ? 'tab-active' : ''
          } ${!hasStatisticsPermission('trends') ? 'tab-disabled opacity-50' : ''}`}
          onClick={() => hasStatisticsPermission('trends') && setActiveModule('trends')}
          disabled={!hasStatisticsPermission('trends')}
          aria-disabled={!hasStatisticsPermission('trends')}
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
        ...
        <button 
          ...
          className={`tab flex items-center gap-2 ${
            pageState.activeModule === 'export' ? 'tab-active' : ''
          } ${!hasStatisticsPermission('export') ? 'tab-disabled opacity-50' : ''}`}
          onClick={() => hasStatisticsPermission('export') && setActiveModule('export')}
          disabled={!hasStatisticsPermission('export')}
          aria-disabled={!hasStatisticsPermission('export')}
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
